...
def load_bundles(
    ...
    if dict(records)[current_path].get("revision") != "10.0.0" or {record.get("revision") for _, record in records} != {
        "1.0.0",
        "1.1.0",
        "2.0.0",
        "3.0.0",
        ...
        raise ValueError("coverage evidence requires the explicit current 10.0.0 release and supported history")
    bundles = []
...
def evaluate(repo_root: Path = REPO_ROOT) -> list[PolicyFailure]:
    ...
    for manifest, protocol, snapshot, analysis in bundles:
        validator = validate_bundle if manifest.get("revision") == "10.0.0" else validate_historical_bundle
