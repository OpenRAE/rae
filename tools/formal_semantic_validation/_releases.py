    ...
    if release.protocol.get("revision") == "2.0.0":
        failures.extend(
            validate_retest_bundle(
                ...
                release.snapshot,
                release.analysis,
                replay_current=manifest.get("revision") == "48.0.0",
            )
        )
    else:
        legacy_manifest = {
...
def validate_retest_bundle(
    ...
    *,
    ...
    if not replay_current and release_revision not in _HISTORICAL_RETEST_REVISIONS:
        return [
            _failure(
                ...
                "only releases 3.0.0 through 47.0.0 can use integrated historical validation",
                snapshot_path,
            )
        ]
    ...
    if release_revision in {"42.0.0", "45.0.0", "46.0.0", "47.0.0", "48.0.0"}:
