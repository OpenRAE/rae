    ...
    if release.protocol.get("revision") == "2.0.0":
        failures.extend(
            validate_retest_bundle(
                ...
                release.snapshot,
                release.analysis,
                replay_current=manifest.get("revision") == "23.0.0",
            )
        )
    else:
        legacy_manifest = {
...
_SUPPORTED_RETEST_REVISIONS = _HISTORICAL_RETEST_REVISIONS | {"23.0.0"}
...
def validate_retest_bundle(
    ...
    *,
    ...
    if not replay_current and release_revision not in _HISTORICAL_RETEST_REVISIONS:
        return [
            _failure(
                ...
                "only releases 3.0.0 through 22.0.0 can use integrated historical validation",
                snapshot_path,
            )
        ]
    ...
    expected_corpus_revision = (
        "3.0.0"
        if release_revision in {"16.0.0", "17.0.0", "18.0.0", "19.0.0", "20.0.0", "21.0.0", "22.0.0", "23.0.0"}
        else "2.0.0"
    )
