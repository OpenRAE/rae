    ...
    if release.protocol.get("revision") == "2.0.0":
        failures.extend(
            validate_retest_bundle(
                ...
                release.snapshot,
                release.analysis,
                replay_current=manifest.get("revision") == "33.0.0",
            )
        )
    else:
        legacy_manifest = {
...
_SUPPORTED_RETEST_REVISIONS = _HISTORICAL_RETEST_REVISIONS | {"33.0.0"}
    ...
    if not replay_current and release_revision not in _HISTORICAL_RETEST_REVISIONS:
        return [
            _failure(
                ...
                "only releases 3.0.0 through 32.0.0 can use integrated historical validation",
                snapshot_path,
            )
        ]
