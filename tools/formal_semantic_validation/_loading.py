...
def load_release_bundles(repo_root: Path = REPO_ROOT) -> list[EvidenceRelease]:
    ...
    if {record.get("revision") for _, record in records} != {"1.0.0", "1.1.0", "1.2.0", "2.0.0", "3.0.0", "4.0.0"}:
...
def load_retest_bundle(
    ...
    if release.manifest.get("revision") != "4.0.0" or release.protocol.get("revision") != "2.0.0":
        raise ValueError("the current formal evidence release must be the explicit 4.0.0 retest")
