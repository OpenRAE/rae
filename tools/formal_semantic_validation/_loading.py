...
def load_release_bundles(repo_root: Path = REPO_ROOT) -> list[EvidenceRelease]:
    ...
    if {record.get("revision") for _, record in records} != {
        ...
        raise ValueError(
            "formal evidence requires every supported historical and current release"
        )
        ...
        for label in ("protocol", "corpus", "snapshot", "analysis"):
            ...
            loaded.append(
                load_bounded_json_object(
                    repo_root, path_value, max_bytes=_MAX_FILE_BYTES
                )
            )
        releases.append(
            EvidenceRelease(
...
def load_retest_bundle(
    ...
    if not releases:
        raise ValueError(
            "the formal semantic-validation index selects no v2 retest release"
        )
    release = max(
        releases, key=lambda item: revision_key(item.manifest.get("revision"))
    )
    if (
        release.manifest.get("revision") != "36.0.0"
        or release.protocol.get("revision") != "2.0.0"
    ):
        raise ValueError(
            "the current formal evidence release must be the explicit 36.0.0 retest"
        )
