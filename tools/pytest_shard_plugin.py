...
def pytest_collection_modifyitems(config: Any, items: list[Any]) -> None:
    ...
    # Record ownership. This hook runs during collection, which under xdist
    # happens in each worker (all of which carry ``workerinput``) over the FULL
    # suite before execution is distributed, so every context computes the
    # identical complete owned set. Write it atomically rather than guarding on
    # ``workerinput`` — guarding on the controller would suppress the write
    # entirely under xdist and leave the required manifest absent.
