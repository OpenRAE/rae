...
def validate_evidence_requirement_relation_carrier(
    ...
    *,
    ...
    for relation in relations:
        ...
        elif _reference_identity(relation.base_scenario_ref) != _reference_identity(scenario_ref):
            raise ValueError("evidence relation base_scenario_ref must match the carrier scenario reference")


def _resolved_authored_requirement(relation: ExperimentEvidenceRequirementRelationModel, scenario: Any) -> Any:
...
def _resolved_scenario_identity(scenario: Any) -> tuple[str, str, str | None, str | None, str | None]:
...
def _dimension_is_stricter(
    ...
    return scoped.loss_disclosure_required and not base_required
    ...
    *,
    ...
    for relation in validated:
        ...
        if relation.relation_kind == "refine":
            comparisons = [
                _dimension_is_stricter(dimension, authored, scoped) for dimension in relation.refinement_dimensions
            ]
