...
def _validate_incumbents(record, context):
    ...
    for result in record.results:
        ...
        if not isinstance(result.payload, ControlSecurityFactModel):
            continue
        fact = result.payload
        ...
        if not any(
            ...
            for decision in relation.sink_decisions
        ):
...
def validate_participant_control_resolved_context(record, resolver: ParticipantControlContextResolver | None) -> None:
