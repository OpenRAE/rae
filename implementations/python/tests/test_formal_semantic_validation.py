...
def test_current_retest_bundle_is_coherent_and_clean() -> None:
    ...
    assert release.manifest["revision"] == "41.0.0"
    assert protocol["revision"] == "2.0.0"
    assert corpus["revision"] == "4.0.0"
    assert snapshot["baseline"]["release_revision"] == "40.0.0"
    assert {item["case_id"] for item in snapshot["deviations"]} == {"semantic-resolved-objective"}
