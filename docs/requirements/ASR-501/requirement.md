...
- IMPLEMENTS → CODE_FILE `tools/schema_coverage/_keys.py` (Closed key sets, bounds, and rule ids for published-schema coverage)
- IMPLEMENTS → CODE_FILE `tools/schema_coverage/_evidence.py` (Validated corpus, formal, and embedded evidence checks)
- IMPLEMENTS → CODE_FILE `tools/schema_coverage/_declaration.py` (Coverage association shape and evidence qualification)
